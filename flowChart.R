...
n <- nrow(cancer)
exclude1 <- cancer %>% filter(is.na(AgeDx)) %>% nrow() # Missing age at diagnosis
exclude2 <- cancer %>% filter(!is.na(AgeDx),
...
grViz(
  glue("digraph my_flowchart {{ 
      ...
      node [fontname = Helvetica, shape = box, width = 4, height = 1]
        ...
        blank1[label = '', width = 0.01, height = 0.01]
        excluded1[label = <Excluded because of<br/>missing age (n={exclude1})>]
