---
title: "SMR Calculations - PR Reference Population"
...
```{r smrs_popEpi_prep, include = FALSE}
...
c <- lexpand(cancer_observed, status = cvdStatus != 0, 
             ...
             aggre = list(agegroup = age, deathyear = per, ydiag, yearDiagCat, deathYearCat, Comorb.cat, maritallabel, Procedure))
...
smr_global <- sir(coh.data = c, coh.obs = 'from0to1', 
    ...
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_global

cat("SMRs by year of death")
...
smr_year <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  print ='deathyear') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_year

cat("SMRs by grouped year of death")


smr_deathYearCat <- smr_year %>%
  mutate(`Year of Death` = case_when(
    smr_year >= 2005 & smr_year <= 2010 ~ "2005-2010",
    smr_year >= 2011 & smr_year <= 2016 ~ "2011-2016",
    smr_year >= 2017 & smr_year <= 2021 ~ "2017-2021"
  )) %>%
...
smr_diag <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  print ='ydiag') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_diag

cat("SMRs by grouped year of diagnosis")
...
smr_diagCat <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  print ='yearDiagCat') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_diagCat


cat("SMRs by age group")
...
smr_agegroup <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  print ='agegroup') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_agegroup # print selects which var to visualise

cat("SMRs by comorbidity status. Filtering for Comorb status != Unknown and year of diagnosis >= 2010")
...
smr_comorb <- sir(coh.data = comorb, coh.obs = 'from0to1', 
                  ...
                  print ='Comorb.cat') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_comorb # print selects which var to visualise

cat("SMRs by marital status, filtering for marital status != Unknown")
...
smr_marital <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  print ='maritallabel') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_marital


cat("SMRs by age group and death year")

# By age group and year of death
smr_facet <- sir(coh.data = c, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2, 
                  ref.rate = 'gen_cvd_rate', 
                  adjust = c('deathyear', 'agegroup'), 
                  ...
                  conf.type = "wald",
                  conf.level = 0.95,
                  print = c('deathyear', 'agegroup')) %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_facet # print selects which var to visualise

# By treatment

smr_proc <- sir(coh.data = c, coh.obs = 'from0to1',
                coh.pyrs = 'pyrs',
                ref.data = gen_2,
                ref.rate = 'gen_cvd_rate',
                adjust = c('deathyear', 'agegroup'),
                ...
                print ='Procedure') %>%
    mutate(sir.lo = qchisq(0.05 / 2, df = 2 * observed) / (2 * expected),
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)); smr_proc
...
print("SMRs by year of diagnosis")

smr_diag

# Line graph by year of diagnosis
ydiag_graph <- ggplot(data = smr_diag, aes(x = ydiag, y = sir)) +
  ...
  geom_text(aes(label = round(sir, 1)), nudge_y = 0.4, nudge_x = -0.15, size = 4, color = "blue") +  # Add labels with a slight upward offset
  ...
  ggtitle("CVD Standardized Mortality Ratios by Year of Diagnosis\nfor BC Survivors in Puerto Rico"); ydiag_graph

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/pr_ref_diagnosisyear_graph.png", plot = ydiag_graph, width = 10, height = 8, dpi = 300,  bg = "white")


print("SMRs by grouped year of diagnosis")

# Horizontal SMRs by grouped year of diagnosis
ggplot(data = smr_deathYearCat, aes(x = SMR, y = `Year of Death`)) +
  ...
  geom_text(aes(label = round(SMR, 1)), nudge_y = 0.2, size = 4, color = "black") +  # Add labels with a slight upward offset
...
print("SMRs by year of death")

smr_year

# Line graph by year of death
ggplot(data = smr_year[-1, ], aes(x = deathyear, y = sir)) +
  geom_line(color = "blue", size = 1.2) +  # Adjust line color and thickness
  geom_point(size = 3, shape = 21, fill = "white", color = "blue") +  # Add points with fill
  geom_ribbon(aes(ymin = sir.lo, ymax = sir.hi), fill = "lightblue", alpha = 0.3) +  # Confidence interval with fill color
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +  # Red dashed line at SMR = 1
 ...
 # scale_y_continuous(breaks = seq(1, max(smr_year$sir.hi, na.rm = TRUE), by = 2), limits = c(1, NA)) +  # Breaks start at 1
  scale_y_continuous(breaks = seq(1, ceiling(max(smr_year[-1, ]$sir.hi, na.rm = TRUE)), by = 2), limits = c(0, NA)) +  # Breaks start at 1
  theme_minimal(base_size = 14) +  # Use a minimal theme with larger font
  theme(
    axis.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),  # Rotate x-axis labels
...
print("SMRs by grouped year of death")

smr_deathYearCat

# smr_deathYearCat$deathYearCat <- factor(smr_deathYearCat$deathYearCat, 
#                                         levels = c("2004-2009", "2010-2015", "2016-2021"), 
#                                         ordered = TRUE)
# Line graph by year of death
ggplot(data = smr_deathYearCat, aes(x = `Year of Death`, y = SMR, group = 1)) +
  geom_line(color = "blue", size = 1.2) + 
  geom_point(size = 3, shape = 21, fill = "white", color = "blue") +
  geom_ribbon(aes(ymin = CI.Lower, ymax = CI.Upper), fill = "lightblue", alpha = 0.3) +  
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) + 
  xlab("Year of Death") +
  ylab("Standardized Mortality Ratio (SMR)") +
 scale_x_discrete( expand = c(0.05, 0.05)) +
  geom_text(aes(label = round(SMR, 1)), position = position_dodge(0), vjust = -1) +
  theme_minimal(base_size = 14) +
  theme(
    axis.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),  # Rotate x-axis labels
    plot.title = element_text(face = "bold", hjust = 0.5)) +
  ggtitle("Standardized Mortality Ratio by Grouped Year of Death")

print("SMRs by age group")
...
obj <- ggplot(data = smr_agegroup[smr_agegroup$agegroup != "18-39", ], aes(x = agegroup, y = sir, fill = agegroup)) +
  ...
  geom_text(aes(label = round(sir, 2)), vjust = -0.5, hjust = -0.6, size = 3, fontface = "bold") +  # Add SMR labels above bars
  xlab("Age Group") +
  ylab("Standardized Mortality Ratio (SMR) (95% CI)") +
  scale_fill_manual(values = rep("indianred", length(unique(smr_agegroup$agegroup)))) + 
  ...
  ggtitle("CVD Standardized Mortality Ratio by Age Group\nfor BC Survivors in Puerto Rico"); obj

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/pr_ref_agegroup_graph.png", plot = obj, width = 10, height = 8, dpi = 300,  bg = "white")
...
print("This next graph is probably not a good choice due to many groups < 10. Shown just so we know")

# Horizontal SMRs by year of diagnosis and agegroup
ggplot(data = smr_facet[smr_facet$agegroup != 0, ], aes(x = sir, y = deathyear)) +
  facet_wrap(~agegroup) +
  geom_point(size = 2, color = "#4E79A7") +  # Blue points for SMR
  geom_errorbarh(aes(xmin = sir.lo, xmax = sir.hi), height = 0.2, color = "black") +  # Horizontal error bars
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +  # Reference line at SMR = 1
  xlab("Standardized Mortality Ratio (SMR)") +
  scale_y_continuous(breaks = smr_facet$deathyear) +  # Add all year labels
 # scale_x_continuous(breaks = seq(1, max(smr_facet$sir.hi, na.rm = TRUE), by = 2), limits = c(1, NA)) +  # Breaks start at 1
  ylab("Age Group") +
  theme_minimal(base_size = 14) +  # Minimal theme
  theme(
    axis.title = element_text(face = "bold"),
  ...
  ggtitle("Standardized Mortality Ratio by Year of Death and Age Group")

print("SMRs by age group")

# Horizontal SMRs by age group
ggplot(data = smr_agegroup[smr_agegroup$agegroup != "18-39", ], aes(x = sir, y = agegroup)) +
  geom_point(size = 4, color = "#4E79A7") +  # Blue points for SMR
  geom_errorbarh(aes(xmin = sir.lo, xmax = sir.hi), height = 0.2, color = "black") +  # Horizontal error bars
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +  # Reference line at SMR = 1
  xlab("Standardized Mortality Ratio (SMR)") +
...
cat("SMRs by treatment")

# By treatment

obj <- ggplot(data = smr_proc, aes(x = Procedure, y = sir, fill = Procedure)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +  # Bar plot with black borders and custom width
  geom_errorbar(aes(ymin = sir.lo, ymax = sir.hi), width = 0.2, color = "black") +  # Add error bars
  geom_text(aes(label = round(sir, 2)), vjust = -0.5, hjust = -0.6, size = 3, fontface = "bold") +  # Add SMR labels above bars
  xlab("Type of Treatment") +
  ylab("Standardized Mortality Ratio (SMR) (95% CI)") +
  scale_fill_manual(values = rep("indianred", length(unique(smr_proc$Procedure)))) + 
  theme_minimal(base_size = 14) +  # Minimal theme with larger font
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +  # Wrap x-axis labels to fit within ~10 characters per line
  theme(element_text(angle = 0, hjust = 0.5),
    axis.title = element_text(face = "bold"),
    plot.title = element_text(face = "bold", hjust = 0.5),
    legend.position = "none"  # Remove legend if not needed
  ) +
  ggtitle("CVD Standardized Mortality Ratio by Type of Treament\nfor BC Survivors in Puerto Rico"); obj

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/pr_ref_procedure_graph.png", plot = obj, width = 10, height = 8, dpi = 300,  bg = "white")
...
```{r sensitivity_latency, include = FALSE}

#Diagnosis Date

c_diag <- lexpand(cancer, status = cvdStatus != 0,
             birth = birth_date,
             entry = DiagnosisDate, # From diagnosis
             exit = FollowUpEnd,
             breaks = list(per = 2004:2022, age = c(0, 40, 50, 60, 70, 80, Inf)),
...
c_12 <- lexpand(cancer_observed, status = cvdStatus != 0, 
             birth = birth_date,
             entry = FollowUpStart, # 12-months latency
             exit = FollowUpEnd, 
             breaks = list(per = 2004:2022, age = c(0, 40, 50, 60, 70, 80, Inf)),
             aggre = list(agegroup = age, deathyear = per, ydiag, yearDiagCat, deathYearCat, Comorb.cat, maritallabel))

c_24 <- lexpand(cancer_observed, status = cvdStatus != 0, 
                birth = birth_date,
                entry = FollowUpStart %m+% years(1), # 24-months latency
                exit = FollowUpEnd, 
                breaks = list(per = 2004:2022, age = c(0, 40, 50, 60, 70, 80, Inf)),
                aggre = list(agegroup = age, deathyear = per, ydiag, yearDiagCat, deathYearCat, Comorb.cat, maritallabel))

smr_global_diag <- sir(coh.data = c_diag, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2, 
                  ref.rate = 'gen_cvd_rate', 
                  adjust = c('deathyear', 'agegroup'), 
...
# 12-month latency period

smr_global_12 <- sir(coh.data = c_12, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2, 
                  ref.rate = 'gen_cvd_rate', 
                  adjust = c('deathyear', 'agegroup'), 
...
# 24-month latency period

smr_global_24 <- sir(coh.data = c_24, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2, 
                  ref.rate = 'gen_cvd_rate', 
                  adjust = c('deathyear', 'agegroup'), 
...
smr_global_diag
# 12 months before follow-up starts
smr_global_12
# 12 months before follow-up starts
smr_global_24
