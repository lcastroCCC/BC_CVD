...
```{r smr_with_follow_up, echo = FALSE, include = FALSE}
...
cat("SMR by grouped year of death")

gen_2_grouped <- gen_2 %>%
    mutate(deathYearCat = case_when(
        deathyear >= 2004 & deathyear <= 2009 ~ "2004-2009",
        deathyear >= 2010 & deathyear <= 2015 ~ "2010-2015",
        deathyear >= 2016 & deathyear <= 2021 ~ "2016-2021"
    )) %>%
    group_by(deathYearCat, agegroup) %>%
    mutate(cvd_rate_grouped = sum(n) / sum(Population)) %>%
    ungroup() 
gen_2_grouped2 <- gen_2 %>%
    mutate(deathYearCat = case_when(
        deathyear >= 2004 & deathyear <= 2009 ~ "2004-2009",
        deathyear >= 2010 & deathyear <= 2015 ~ "2010-2015",
        deathyear >= 2016 & deathyear <= 2021 ~ "2016-2021"
    ...
    group_by(deathYearCat, agegroup) %>%
    summarise(cvd_rate_grouped = sum(n) / sum(Population)) 

# By grouped year of death
c2 <- lexpand(cancer_observed, status = cvdStatus != 0, 
             birth = birth_date,
             entry = FollowUpStart, # Without those that did not meet follow-up
             exit = FollowUpEnd, 
             breaks = list(per = 2004:2022, age = c(0, 40, 50, 60, 70, 80, Inf)),
             aggre = list(agegroup = age, deathYearCat))

smr_deathYearCat <- sir(coh.data = c2, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2_grouped2, 
                  ref.rate = 'cvd_rate_grouped', 
                  adjust = c('deathYearCat', 'agegroup'), 
                  test.type = "homogeneity",
                  conf.type = "wald",
                  conf.level = 0.95,
                  print = c('deathYearCat', 'agegroup')); smr_deathYearCat
#testing
gen_2_grouped2[gen_2_grouped2$agegroup == 80,]
c2[deathYearCat != "2022-2024" & agegroup == 80, ]

smr_deathYearCat2 <- sir(coh.data = c, coh.obs = 'from0to1', 
                  coh.pyrs = 'pyrs', 
                  ref.data = gen_2, 
                  ref.rate = 'gen_cvd_rate', 
                  adjust = c('deathyear', 'agegroup'), 
                  ...
                  conf.type = "wald",
                  conf.level = 0.95,
                  print ='deathyear'); smr_deathYearCat2
...
cat("Extra for graphs: SMR by age group and death year")
...
smr_facet <- sir(coh.data = c, coh.obs = 'from0to1', 
                  ...
                  conf.type = "wald",
                  conf.level = 0.95,
                  print = c('deathyear', 'agegroup')); smr_facet # print selects which var to visualise
```
...
```{r year_graphs, echo = FALSE, warning = FALSE}
...
smr_deathYearCat$deathYearCat <- factor(smr_deathYearCat$deathYearCat, 
                                        ...
                                        ordered = TRUE)
# Line graph by year of death
ggplot(data = smr_deathYearCat, aes(x = deathYearCat, y = sir, group = 1)) +
  geom_line(color = "blue", size = 1.2) +  # Adjust line color and thickness
  geom_point(size = 3, shape = 21, fill = "white", color = "blue") +  # Add points with fill
  geom_ribbon(aes(ymin = sir.lo, ymax = sir.hi), fill = "lightblue", alpha = 0.3) +  # Confidence interval with fill color
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +  # Red dashed line at SMR = 1
  xlab("Year of Death") +
  ylab("Standardized Mortality Ratio (SMR)") +
  scale_x_discrete(limits = c("2004-2009", "2010-2015", "2016-2021"), expand = c(0.05, 0.05)) +  # Fix axis limits with no padding
  scale_y_continuous(breaks = seq(1, ceiling(max(smr_deathYearCat$sir.hi, na.rm = TRUE))), limits = c(0, NA)) +  # Breaks start at 1
  theme_minimal(base_size = 14) +  # Use a minimal theme with larger font
  theme(
    axis.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),  # Rotate x-axis labels
...
print("This next graph is probably not a good choice. Shown just so we know")

# Horizontal SMRs by year of diagnosis and agegroup
ggplot(data = smr_facet, aes(x = sir, y = deathyear)) +
  facet_wrap(~agegroup) +
...
ggplot(data = smr_deathYearCat, aes(x = sir, y = deathYearCat)) +
  geom_point(size = 4, color = "#4E79A7") +  # Blue points for SMR
  geom_errorbarh(aes(xmin = sir.lo, xmax = sir.hi), height = 0.2, color = "black") +  # Horizontal error bars
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +  # Reference line at SMR = 1
  xlab("Standardized Mortality Ratio (SMR)") +
