...
suppress_values <- function(data) {
  data %>%
    mutate(
      ...
      Expected = ifelse(Observed == "<10", "-", comma(as.numeric(Expected), accuracy = 0.01)),
...
create_gt_table <- function(df, groupname_col = NULL) {
  ...
  table <- table |> 
    ...
    tab_source_note(
      source_note = "Note. SMR = Standarized Mortality Ratio. For Race/Ethnicity, AANHPI & AIAN encompasses: Asian American, Native Hawaiian and other Pacific Islanders along with American Indian/Alaska Natives. SMRs were calculated as the observed
number of cardiovascular disease deaths among breast cancer patients divided by the expected number of cardiovascular disease deaths in the race- and ethnic-matched general female U.S. population. SMR calculations for results with <10 events were suppressed."
    )
...
read.delim("C:/Users/lcastro/OneDrive - Centro Comprensivo de Cancer UPR/projects_shared/BBC/Project_Dra. Carola/Data/Registro Central de Cáncer/us_reference.txt")
}))
...
us_smr_general <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_general.txt")
}, error = function(e) {
...
figure <- ggplot(merged_general, aes(y = fct_rev(race_eth), x = as.numeric(SMR))) +
...
us_sir_general <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/sir_general.txt")
}, error = function(e) {
...
us_smr_deathyear <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_deathyear.txt")
}, error = function(e) {
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/g_deathyear.png", plot = figure1, width = 10, height = 8, dpi = 300,  bg = "white")
...
us_smr_diagnosisyear <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_diagnosisyear.txt")
}, error = function(e) {
...
figure2 <- ggplot(merged_diagnosisyear, aes(x = `Year of Diagnosis`, y = as.numeric(SMR), color = race_eth, fill = race_eth, group = race_eth)) +
 ...
 # geom_ribbon(aes(ymin = as.numeric(CI.Lower), ymax = as.numeric(CI.Upper)), alpha = 0.2, color = NA) +  # Confidence interval ribbons
  ...
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 0.5) +  # Horizontal line at SMR = 1
  labs(
    title = "CVD Standardized Mortality Ratios (SMRs) Over Time by Race/Ethnicity",
    x = "Year of Diagnosis",
    y = "SMR (95% CI)",
    color = "Race/Ethnicity",
    fill = "Race/Ethnicity") +
...
```{r pr_diagnosisyear_graph}

# Create the ggplot
figure2 <- ggplot(pr_smr_diagnosisyear, aes(x = `Year of Diagnosis`, y = round(as.numeric(SMR), 1))) +
  geom_line(size = 1) +  # Line for SMR over years
  geom_ribbon(aes(ymin = as.numeric(CI.Lower), ymax = as.numeric(CI.Upper)), alpha = 0.2, color = NA) +  # Confidence interval ribbons
  theme_minimal() +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 0.5) +  # Horizontal line at SMR = 1
  ...
  geom_label(aes(label = round(SMR, 1)), position = position_dodge(0), vjust = -0.5) +
  ...
  ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/pr_diagnosisyear_graph.png", plot = figure2, width = 10, height = 8, dpi = 300,  bg = "white")
...
gt_g_diagnosisyear <- create_gt_table(merged_g_diagnosisyear_wider); gt_g_diagnosisyear
...
us_smr_dx_agegroup <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_dx_agegroup.txt")
}, error = function(e) {
...
# Creating gt table
gt_dx_agegroup <- create_gt_table(merged_dx_agegroup_wider); gt_dx_agegroup

# Sys.setenv(CHROMOTE_CHROME = "/Applications/Brave\ Browser.app/Contents/MacOS/Brave\ Browser")

# Saving table

gtsave(gt_agegroup, "~/Downloads/Statistical Analysis/Outputs/Tables/Dx_AgeGroup_table.html")

webshot::webshot("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Tables/Dx_AgeGroup_table.html",
                 path.expand("~/Downloads/Statistical Analysis/Outputs/Tables/Dx_AgeGroup_table.png"),
                 vwidth = 2000,
                 vheight = 400)
...
# Create the ggplot
figure3 <- ggplot(merged_agegroup[merged_agegroup$`Age Group` != "18-39", ], aes(x = `race_eth`, y = as.numeric(`SMR`), fill = `race_eth`)) +
  ...
  ggtitle("CVD Standardized Mortality Ratio by Age Group\nfor BC Survivors in Puerto Rico and US Race/Ethnic Groups"); figure3
...
# Create the ggplot
figure3 <- ggplot(pr_smr_agegroup[pr_smr_agegroup$`Age Group` != "18-39", ], aes(fill = `Age Group`, x = `Age Group`, y = as.numeric(`SMR`))) +
    geom_bar(stat = "identity") + 
    labs(x = "Age Group",
        y = "Standardized Mortality Ratio (SMR)",
        fill = "Age Group") +
    theme_minimal() +
    geom_text(aes(label = round(`SMR`,1)), vjust = -0.5, size = 3) +  # Add SMR value above each bar
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/pr_agegroup_graph.png", plot = figure3, width = 10, height = 8, dpi = 300,  bg = "white")
...
us_smr_agegroup <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_agegroup.txt")
}, error = function(e) {
...
merged_agegroup <- recode_race_eth(merged_agegroup)
...
us_smr_stage <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_stage.txt")
}, error = function(e) {
...
us_smr_procedure <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_procedure.txt")
}, error = function(e) {
...
us_smr_proc_stage <- suppressWarnings(tryCatch({
  read.delim("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/smr_procedure_stage.txt")
}, error = function(e) {
...
figure1 <- ggplot(merged_proc_stage2[merged_proc_stage2$Procedure != "Other", ], aes(y = Stage, x = as.numeric(SMR))) +
    geom_point(size = 0.5, position = position_dodge(width = 0.5)) +
    geom_errorbar(
   ...
   geom_text(aes(label = round(as.numeric(SMR), 2)), 
            vjust = -1.7, size = 2, fontface = "bold", color = "grey30") +
  theme_minimal() +
    labs(y = "", x = "SMR (95% CI)") +
    theme(
        text = element_text(size = 7), 
  ...
  scale_x_continuous(limits = c(-5,10), breaks = c(-5, 1, 5.0, 10)); figure1

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Graphs/figure1.png", plot = figure1, width = 8, height = 6, dpi = 1200,  bg = "white")
...
gtsave(gt_table1, "~/Downloads/Statistical Analysis/Outputs/Tables/Table_1.pdf")
...
# Saving table
gtsave(gt_sir_general, "~/Downloads/Statistical Analysis/Outputs/Tables/SIR_general_table.html")

webshot::webshot("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Tables/SIR_general_table.html",
                 path.expand("~/Downloads/Statistical Analysis/Outputs/Tables/SIR_general_table.png"),
                 vwidth = 800,
                 vheight = 400)
...
us_sir_general_cl_df <- suppressWarnings(tryCatch({
  read.delim2("C:/Users/Genesis Rodriguez/Downloads/Statistical Analysis/Inputs/sir_general_case_listing.txt")
}, error = function(e) {
read.delim2("C:/Users/lcastro/OneDrive - Centro Comprensivo de Cancer UPR/projects_shared/BBC/Project_Dra. Carola/Data/Inputs/sir_general_case_listing.txt")
  } )) %>%
  ...
  filter(Event.Number == "Index Record",
         Race.Ethnicity.CVD != " ")
# us_sir_general_cl_df <- us_sir_general_cl_df %>%
#   filter(Survival.months < 12)
...
us_sir_general_cl <- us_sir_general_cl_df %>%
  ...
  arrange(Cause.of.Death.CVD.recode); us_sir_general_cl
...
cuminc_fit <- cuminc(FollowUpYears, fstatus = , group = cancer_observed)
...
sum_observed_columns(merged_agegroup_wider)
