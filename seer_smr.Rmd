...
add_p(
  ...
  as_gt() |>
    tab_source_note(
      source_note = "P-values were calculated using Welch's one-way test for continuous variables to account for unequal variances. For categorical variables, the chi-squared test was used unless cell counts were low, in which case Fisher's exact test was applied to obtain a simulated p-value."
    )
...
gt_pr_general <- sir(coh.data = c, coh.obs = 'from0to1', 
    ...
    sir.hi = qchisq(1 - 0.05 / 2, df = 2 * (observed + 1)) / (2 * expected)) %>% gt()
...
f_deathYearCat <- ggplot(data = smr_deathYearCat, aes(y = fct_rev(`Year of Death`), x = SMR, group = 1)) +
  ...
  xlab("Standardized Mortality Ratio (SMR) (95% CI)") +
...
f_ydiag <- ggplot(data = smr_ydiag, aes(x = `Year of Diagnosis`, y = `Standardized Mortality Ratio (SMR)`)) +
  geom_line(color = "#4E79A7", size = 1.2) +  # Adjust line color and thickness
  geom_point(size = 3, shape = 21, fill = "white", color = "#4E79A7") +  # Add points with fill
  geom_ribbon(aes(ymin = `Lower 95% CI`, ymax = `Upper 95% CI`), fill = "lightblue", alpha = 0.3) +  # Confidence interval with fill color
  geom_text(aes(label = round(`Standardized Mortality Ratio (SMR)`, 1)), nudge_y = 0.7, nudge_x = -0.1, size = 4, color = "black") +  # Add labels with a slight upward offset
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +  # Red dashed line at SMR = 1
  xlab("Year of Diagnosis") +
  ylab("Standardized Mortality Ratio (SMR) (95% CI)") +
  scale_x_continuous(breaks = smr_ydiag$`Year of Diagnosis`) +  # Add all year labels
  scale_y_continuous(breaks = seq(1, max(smr_ydiag$`Upper 95% CI`, na.rm = TRUE), by = 2), limits = c(1, NA)) +  # Breaks start at 1
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/pr_diagnosisyear_graph.tif", plot = f_ydiag, width = 10, height = 6, dpi = 300,  bg = "white")

f_ydiag2 <- ggplot(data = smr_ydiag, aes(y = `Year of Diagnosis`, x = `Standardized Mortality Ratio (SMR)`)) +
  geom_point(size = 4, color = "#4E79A7") +  # Blue points for SMR
  geom_errorbarh(aes(xmin = `Lower 95% CI`, xmax = `Upper 95% CI`), height = 0.2, color = "black") +  # Horizontal error bars
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +  # Reference line at SMR = 1
  xlab("Standardized Mortality Ratio (SMR) (95% CI)") +
  scale_y_reverse(n.breaks = length(smr_ydiag$`Year of Diagnosis`)) +  # Reverse the y-axis order for continuous variables
  scale_x_continuous(breaks = seq(1, max(smr_ydiag$`Upper 95% CI`, na.rm = TRUE), by = 2), limits = c(1, NA)) +  # Breaks start at 1
...
f_ydiagcat <- ggplot(data = smr_diagCat, aes(x = SMR, y = fct_rev(`Year of Diagnosis`))) +
  geom_point(size = 4, color = "#4E79A7") +  # Blue points for SMR
  geom_errorbarh(aes(xmin = CI.Lower, xmax = CI.Upper), height = 0.2, color = "black") +
  geom_text(aes(label = round(SMR, 1)), nudge_y = 0.2, size = 4, color = "black") + 
...
f_pragegroup <- ggplot(data = smr_agegroup[smr_agegroup$AgeDxCat != "18-39", ], aes(x = AgeDxCat, y = sir, fill = AgeDxCat)) +
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/pr_agegroup_diagnosis_graph.tif", plot = f_pragegroup, width = 11, height = 5.5, dpi = 300,  bg = "white")
...
f_prtreatment <- ggplot(data = smr_treatment, aes(x = Procedure, y = sir, fill = Procedure)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +  # Bar plot with black borders and custom width
  geom_errorbar(aes(ymin = sir.lo, ymax = sir.hi), width = 0.2, color = "black") +  # Add error bars
  geom_text(aes(label = sprintf("%.1f", sir)), vjust = -0.5, hjust = -0.6, size = 5) +  # Add SMR labels above bars
  xlab("Initial Treatment Type") +
  ylab("Standardized Mortality Ratio (SMR) (95% CI)") +
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/pr_treatment.tif", plot = f_prtreatment, width = 11, height = 6, dpi = 300,  bg = "white")
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/pr_comorb.tif", plot = f_comorb, width = 11, height = 6, dpi = 300,  bg = "white")
...
f_general <- ggplot(merged_general, aes(y = fct_rev(race_eth), x = as.numeric(SMR))) +
    ...
    theme_minimal() +
    scale_x_continuous(limits = c(0.60, 1.5)) +
    geom_vline(xintercept = 1, linetype = "solid", color = "red", size = 0.2) +
    geom_text(aes(label = sprintf("%.1f", as.numeric(SMR))), position = position_dodge(0), vjust = -1) +
    labs(x = "Standardized Mortality Ratio (SMR) (95% CI)", y = "Race/Ethnicity") +
    theme(axis.text.x = element_text(hjust = 1)) +
    scale_y_discrete(labels = function(x) str_wrap(x, width = 20))

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/general_graph.tif", plot = f_general, width = 10, height = 8, dpi = 300,  bg = "white")
...
f_deathyear <- ggplot(merged_deathyear, aes(x = `Year of Death`, y = as.numeric(SMR), color = race_eth, fill = race_eth, group = race_eth)) +
 ...
 # geom_ribbon(aes(ymin = as.numeric(CI.Lower), ymax = as.numeric(CI.Upper)), alpha = 0.2, color = NA) +  # Confidence interval ribbons
  ...
  labs(
    x = "Year of Death",
    y = "Standardized Mortality Ratio (SMR) (95% CI)",
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/grouped_deathyear_graph.tif", plot = f_deathyear, width = 10, height = 8, dpi = 300,  bg = "white")
...
f_g_deathyear <- ggplot(merged_g_deathyear, aes(y = `Year of Death`, x = as.numeric(SMR))) +
  ...
  theme_bw() +
    labs(x = "", y = "Standardized Mortality Ratio (SMR) (95% CI)") +
    theme(
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/g_deathyear.tif", plot = f_g_deathyear, width = 10, height = 8, dpi = 300,  bg = "white")
...
f_diagnosisyear <- ggplot(merged_diagnosisyear, aes(x = `Year of Diagnosis`, y = as.numeric(SMR), color = race_eth, fill = race_eth, group = race_eth)) +
  geom_line(size = 1) +  # Line for SMR over years
  theme_minimal() +
  geom_hline(yintercept = 1, linetype = "solid", color = "red", size = 0.5) +  # Horizontal line at SMR = 1
  labs(
    x = "Year of Diagnosis",
    y = "Standardized Mortality Ratio (SMR) (95% CI)",
    color = "Race/Ethnicity",
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/diagnosisyear_graph.tif", plot = f_diagnosisyear, width = 10, height = 8, dpi = 300,  bg = "white")
...
f_g_diagnosisyear <- ggplot(merged_g_diagnosisyear, aes(y = fct_rev(`Year of Diagnosis`), x = as.numeric(SMR))) +
    ...
    geom_errorbar(
        ...
        width = 0.2, position = position_dodge(width = 0.5)) +
    geom_vline(xintercept = 1, linetype = "solid", color = "red", size = 0.2) +
      geom_text(aes(label = round(`SMR`,1)), vjust = -0.5, size = 3) +
    facet_wrap(~ race_eth, ncol = 2) +
    theme_bw() +
    labs(y = "Year of Diagnosis", x = "Standardized Mortality Ratio (SMR) (95% CI)") +
    theme(
        strip.background = element_rect(fill = NA, color = NA),
        legend.position = "none",
        panel.spacing = unit(0.5, "lines"))
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/g_deathyear.tif", plot = f_g_diagnosisyear, width = 10, height = 8, dpi = 300,  bg = "white")
...
# Create the ggplot
f_dx_agegroup <- ggplot(merged_dx_agegroup, aes(x = `race_eth`, y = as.numeric(`SMR`), fill = `race_eth`)) +
  ...
  scale_y_continuous(
        breaks = scales::pretty_breaks(n = 6), limits = c(0, 3))

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/agegroup_diagnosis_graph.tif", plot = f_dx_agegroup, width = 10, height = 8, dpi = 300,  bg = "white")
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/agegroup_deathyear_graph.tif", plot = f_agegroup, width = 10, height = 8, dpi = 300,  bg = "white")
...
merged_stage_wider <- merged_stage %>%
  mutate(across(c(Expected:CI.Upper), ~ ifelse(Stage != "Unknown/unstaged", format(round(as.numeric(.), 1), nsmall = 1), "-")),
...
f_stage <- ggplot(merged_stage[merged_stage$Stage != "Unknown/unstaged",], aes(y = fct_rev(Stage), x = as.numeric(SMR))) +
  ...
  geom_errorbar(
    ...
    width = 0.2, position = position_dodge(width = 0.5)) +
  geom_vline(xintercept = 1, linetype = "solid", color = "red", size = 0.2) +
  geom_text(aes(label = round(as.numeric(`SMR`), 1)), vjust = -0.5, size = 3) +
  facet_wrap(~ race_eth, ncol = 2) +
  theme_bw() +
  labs(y = "Stage at Diagnosis", x = "Standardized Mortality Ratio (SMR) (95% CI)") +
  theme(
    strip.background = element_rect(fill = NA, color = NA),
    legend.position = "none",
    panel.spacing = unit(0.5, "lines"))

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/us_stage_graph.tif", plot = f_stage, width = 8, height = 6.5, dpi = 1200,  bg = "white")
...
f_treatment <- ggplot(merged_procedure, aes(y = fct_rev(Treatment), x = as.numeric(SMR))) +
  geom_point(size = 0.5, position = position_dodge(width = 0.5)) +
  geom_errorbar(
    aes(xmin = as.numeric(CI.Lower), xmax = as.numeric(CI.Upper)),
    width = 0.2, position = position_dodge(width = 0.5)) +
  ...
  facet_wrap(~ race_eth, ncol = 2) +
  theme_bw() +
  labs(y = "Initial Treatment Type", x = "Standardized Mortality Ratio (SMR) (95% CI)") +
  theme(
    strip.background = element_rect(fill = NA, color = NA),
    legend.position = "none",
    panel.spacing = unit(0.5, "lines"))

ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/us_treatment_graph.tif", plot = f_treatment, width = 8, height = 6.5, dpi = 1200,  bg = "white")
...
f_proc_stage <- ggplot(merged_proc_stage2[merged_proc_stage2$Procedure != "Other", ], aes(y = Stage, x = as.numeric(SMR))) +
  ...
  theme_minimal() +
    labs(y = "", x = "Standardized Mortality Ratio (SMR) (95% CI)") +
    theme(
      #  text = element_text(size = 7), 
...
ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/proc_stage_graph.tif", plot = f_proc_stage, width = 8, height = 6.5, dpi = 1200,  bg = "white")
...
#image_read("~/Downloads/Statistical Analysis/Outputs/Figures/Flowchart.tif")
...
# PR SMR by agegroup
cat(paste("\n Table", t + 1, " CVD SMRs by Age at Diagnosis for BC Survivors in Puerto Rico Using PR Population as Referent\n"))
t <- t + 1
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Age at Diagnosis for BC survivors in Puerto Rico Using PR population as Referent\n"))
f <- f + 1
...
# Overall SMR
cat(paste("\n Table", t + 1, " CVD SMRs for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US Population as Referent\n"))
t <- t + 1
...
# Overall SMR figure
cat(paste("\n Figure", f + 1, " CVD SMRs for BC survivors in PR Hispanic and US Race/Ethnicity Groups using US population as referent\n"))
f <- f + 1
f_general
...
# SMRs by year of diagnosis
cat(paste("\n Table", t + 1, " CVD SMRs by Year of Diagnosis for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
t <- t + 1
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Year of Diagnosis for BC survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
f <- f + 1
...
cat(paste("\n Table", t + 1, " CVD SMRs by Age at Diagnosis for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
t <- t + 1
gt_dx_agegroup
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Age at Diagnosis for BC survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
f <- f + 1
f_dx_agegroup
...
# SMRs by stage
cat(paste("\n Table", t + 1, " CVD SMRs by Stage at Diagnosis for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
t <- t + 1
gt_stage
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Stage at Diagnosis for BC survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
f <- f + 1
f_stage
...
cat(paste("\n Table", t + 1, " CVD SMRs by Initial Treatment for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
t <- t + 1
gt_procedure
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Initial Treatment for BC survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
f <- f + 1
f_treatment
...
cat(paste("\n Table", t + 1, " CVD SMRs by Stage and Treatment for BC Survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
t <- t + 1
gt_proc_stage
...
cat(paste("\n Figure", f + 1, " CVD SMRs by Stage and Treatment for BC survivors in PR Hispanic and US Race/Ethnicity Groups Using US population as Referent\n"))
f <- f + 1
f_proc_stage
...
# 
# # knitr::plot_crop("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/pr_hispanic_forest.tif")
# 
# ggsave("/Users/genesisrodriguez/Downloads/Statistical Analysis/Outputs/Figures/forest_figure.tif", plot = forest_figure, width = 30, height = 8, dpi = 400,  bg = NULL)
# 
# beepr::beep(1)
