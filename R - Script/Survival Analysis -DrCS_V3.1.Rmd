...
data <- data %>%
  mutate(mm_dx = as.numeric(ifelse(is.na(mm_dx), 6, mm_dx)),
         ...
         time = FollowUpYears,
         AgeFU = AgeDx + time,
         maritallabel = factor(str_to_title(maritallabel)),
...
data$yearDiagCat <- cut(data$ydiag, 
                   ...
                   right = TRUE)
#datatest <- data %>% select(Vital, ydiag, mm_dx, FollowUpStart, FollowUpEnd, yy_dlc, mm_dlc, FollowUpYears, time) 
# data <- data %>%
...
# dataNO <- data %>%
#   filter(time <= 0) # Person did not meet follow-up criteria
# nrow(dataNO); nrow(data) # No follow-up period vs everyone
# summary(dataNO$FollowUpYears)
# table(dataNO$Vital)
...
data %>%
  select(c(Vital, Procedure, StageAtDx, AgeDxCat, ER, PR, HER2.cat, CsumCat, `Myocardial Infarction`, `Congestive heart failure`, `Peripheral vascular disease`, `Cerebrovascular disease`, `Chronic pulmonary disease`, `Diabetes without chronic complication`, `Diabetes with chronic complication`, HealthRegion)) %>%
  filter(Vital != "Alive", !is.na(AgeDxCat)) %>% 
 tbl_summary(by = Vital,
              label = c(Procedure ~ "Treatment",
                        StageAtDx ~ "Stage at Diagnosis",
                        AgeDxCat ~ "Age at Diagnosis",
                        ER ~ "ER Assay",
                        PR ~ "PR Assay",
                        HER2.cat ~ "HER2", 
...
*Figure 12: CVD Specific Mortality Survival Curve* 
...
*Figure 13: CVD Specific Mortality Survival Curve by Treatment* 
...
*Figure 14: CVD Specific Mortality Survival Curve by Surgery Treatment* 
...
*Figure 15: CVD Specific Mortality Survival Curve by Marital Status* 
...
*Figure 16: CVD Specific Mortality Survival Curve by Year of Diagnosis* 
...
*Figure 17: CVD Specific Mortality Survival Curve by Comorbidity* 
...
*Figure 15: Cumulative incidence functions (1 = Cardiac death ; 2 = Other cause death)* 
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
*Figure 16: Cumulative incidence functions (1 = Cardiac death ; 2 = Breast Cancer death, 3 = Other Cause death)* 
...
*Figure 17: CIF for CVD Mortality by Treatment* 
...
*Figure 19: CIF for CVD Mortality by Comorbidity* 
...
*Figure 20: CIF for BC Mortality by Comorbidity* 
...
*Figure 21: Cumulative incidence curves for CVD and BC Mortality by Comorbidity* 
...
*Figure 22: Cumulative incidence curves for CVD Mortality by Stage at Diagnosis* 
...
*Figure 23: Cumulative incidence curves for BC Mortality by Stage at Diagnosis* 
...
*Figure 24: Cumulative incidence curves for CVD and BC Mortality by Stage at Diagnosis* 
...
*Figure 25: Subdistribution HR (95% CI) by Stage at Diagnosis* 
...
*Figure 26: Subdistribution HR (95% CI) by Treatment* 
...
*Figure 27: Subdistribution HR (95% CI) by Comorbidity* 
