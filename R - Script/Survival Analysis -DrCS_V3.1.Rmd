...
data %>%
 ...
 modify_column_hide(columns = "stat_4") %>%
  as_gt() %>%
...
*Figure 13: Overall Mortality Survival Curve by Treatment* 
...
*Figure 14: All Cause Mortality Survival Curve by Surgery Treatment* 
...
*Table 5: All Cause Mortality Survival Confidence Intervals Table by Surgery Treatment* 
...
*Table 6: Cumulative Incidence of Death (CVD Death vs Other)*
...
tidycmprsk::cuminc(Surv(time, statusV)~1, data) %>% 
  ...
  labels = c("Cardiac death", "Breast cancer death", "Other cause of death"))
...
*Table 7: Cumulative Incidence of Death (including BC mortality)*
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
tidycmprsk::cuminc(Surv(time, status2) ~ AgeCat, data) %>% 
...
tidycmprsk::cuminc(Surv(time, statusV) ~ StageAtDx , data) %>% 
 ...
 #add_risktable() +
  ...
  xlim(c(0,20)) + scale_color_manual(values = c("red", "pink"),
...
*Table 8: Hazard ratios (95% CI) from Subdistribution Hazard Model for Cardiac Death* 
```{r echo=FALSE, message=FALSE, warning=FALSE}
