...
filtered_levels <- levels(droplevels(data$Procedure[!data$Procedure %in% c("Unknown", "No treatment")]))

km_plot2 <- ggsurvplot(
    km_fit2, 
    data = data %>% filter(!Procedure %in% c("Unknown", "No treatment")),  # Filtered data
...
*Table 5: All Cause Mortality Survival Confidence Intervals Table by Treatment* 
...
*Table 6: All Cause Mortality Survival Confidence Intervals Table by Surgery Treatment* 
...
tidycmprsk::cuminc(Surv(time, status2) ~ AgeCat, data) %>% 
  ggcuminc(outcome = c( "1"), aes(color = AgeCat), size = 1)+
