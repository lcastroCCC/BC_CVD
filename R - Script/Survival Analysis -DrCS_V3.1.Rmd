...
data <- read.xlsx('C:/Users/lcastro/OneDrive - Centro Comprensivo de Cancer UPR/projects_shared/BBC/Project_Dra. Carola/Data/datosCarola_invasive_seq0&1_NoID.xlsx',sheet = 1)
...
data$HealthRegion <- toupper(data$HealthRegion)
...
cuminc(Surv(time, status2) ~ Comorb.cat , data) %>% 
  ggcuminc(outcome = c( "1"),
           linetype_aes = TRUE, size = 1)+
 #add_confidence_interval() +
 #add_risktable() +
  ...
  labs(
    x = "Survival Time (Years)",
    title = 'CIF for CVD by Comorbidity'
  ) + 
  xlim(c(0,20)) +  scale_linetype_manual(values=c( 1:9))
