...
cancer %>%   
  ...
  select(Vital, AgeDx, ydiag, yy_dlc, SurvivalTime, Surgery, Radiotherapy, Chemotherapy) %>% View()

# Persons observated #21,596
...
cvm_by_year <- cancer_observated %>%
  ...
  mutate(bc_rate = (cvd_deaths_BC / PersonYears) * 1000)  # Calcular tasa por 1000 person-years
cvm_by_year
  
# CVM by periods
cvm_by_period <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
...
cvm_by_agegroup1 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
...
cvm_by_agegroup2 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
...
cvm_by_year_agegroup1 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
  rename(AgeGroup = AgeDeathCat) %>% 
...
cvm_by_year_agegroup2 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
  rename(AgeGroup2 = AgeDeathCat2) %>% 
...
cvm_by_period_agegroup1 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
  mutate(Period = case_when(yy_dlc > 2004 & yy_dlc < 2010 ~ "2005-2009",
                            yy_dlc > 2009 & yy_dlc < 2015 ~ "2010-2014",
                            yy_dlc > 2014 & yy_dlc < 2020 ~ "2015-2019",
...
# CVM by year and age categories (18-59, 60-69, 70-79, 80+)
cvm_by_period_agegroup2 <- cancer_observated %>%
  filter(Vital == "CVD death", yy_dlc <= 2024) %>%  # Filtrar muertes por CVD en el período de seguimiento
  mutate(Period = case_when(yy_dlc > 2004 & yy_dlc < 2010 ~ "2005-2009",
                            yy_dlc > 2009 & yy_dlc < 2015 ~ "2010-2014",
                            yy_dlc > 2014 & yy_dlc < 2020 ~ "2015-2019",
