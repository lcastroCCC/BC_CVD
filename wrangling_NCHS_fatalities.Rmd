...
mort <- mort %>%
  ...
  mutate(across(c(`ICD Code (10th Revision)`, `1st Condition`, `2nd Condition`, `3rd Condition`), ~ sub("^(.{3})(.)", "\\1.\\2", .))) %>% # Insert a period after the third character for any 4-character string in `1st Condition`
  mutate(bcAny = ifelse(grepl("^C(50)", `1st Condition`)|
           grepl("^C(50)", `2nd Condition`)|
           grepl("^C(50)", `3rd Condition`), TRUE, FALSE), # Diagnosis in any level
         sameLead = `ICD Code (10th Revision)` == `1st Condition`) # Variable that checks if ICD 10 and 1st Condition are the same
...
test <- mort %>%
  filter(grepl("^G(30)", `ICD Code (10th Revision)`))
...
test2 <- mort %>%
  filter(grepl("^G(30)", `1st Condition`))
#Codes included check
table(test$`ICD Code (10th Revision)`)
...
test <- mort %>%
 # filter(grepl("^I(00|0[1-9]|11|13|2[0-9]|3[0-9]|4[0-9]|50|51)", `ICD Code (10th Revision)`))
  filter(grepl("^I(0|11|13|2|3|4|50|51)", `ICD Code (10th Revision)`))
...
test2 <- mort %>%
   filter(grepl("^I(0|11|13|2|3|4|50|51)",, `1st Condition`))
...
cat(sort(unique(test$`1st Condition`)), sep = " ")
test <- mort %>%
  filter(`Record Type` == 1, # Resident of P.R.
         `Detail Age Unit` == 1, # Age in years
         `Detail Age` >= 18, # 18 years +
...
sort(unique(test$`1st Condition`))
